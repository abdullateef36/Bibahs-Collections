...
export default function ShoeCard({ item, isAdmin = false, onEdit, onDelete }: ShoeCardProps) {
  const { user } = useUser();
  ...
  return (
    <motion.div
      onClick={() => router.push(`/shoes/${item.id}`)}
    ...
    >
      ...
      <div className="p-3">
        ...
        <p className="text-xs uppercase text-white/40 tracking-[0.3em] mt-1">
          {item.specifications.material} • {item.specifications.style}
        </p>
