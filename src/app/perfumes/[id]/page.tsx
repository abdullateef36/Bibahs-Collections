...
export default function PerfumeDetailPage() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const data = await perfumeService.getPerfume(id);
        if (data) {
          setItem(data);
        } else {
            ...
            <div className="border border-white/10 rounded-2xl p-5 bg-white/5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-white/70">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <span>{item.specifications.concentration}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Scent Family</span>
                  <span>{item.specifications.scentFamily}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Top Notes</span>
                  <span>{item.specifications.topNotes}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Middle Notes</span>
                  <span>{item.specifications.middleNotes}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Base Notes</span>
                  <span>{item.specifications.baseNotes}</span>
                </div>
                {item.specifications.longevity && (
                  <div className="flex justify-between gap-3">
                    <span className="text-white/50">Longevity</span>
                    <span>{item.specifications.longevity}</span>
                  </div>
                )}
              </div>
            </div>
