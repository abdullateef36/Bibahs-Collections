...
export default function ShoesPage() {
  ...
  const { shoes, loading, error, refresh } = useShoes(48);

  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<Shoe | null>(null);

  const handleCreate = async (data: ShoeFormData) => {
    try {
      await shoeService.createShoe(data);
      setShowForm(false);
      refresh();
    } catch (err) {
  ...
  const handleUpdate = async (data: ShoeFormData) => {
    if (!editingItem) return;

    try {
      await shoeService.updateShoe(editingItem.id, data);
      setShowForm(false);
      setEditingItem(null);
      refresh();
    } catch (err) {
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              ...
              <h1 className="font-heading text-4xl sm:text-5xl text-white mt-2">Shoes</h1>
              <p className="font-body text-white/60 mt-3 max-w-xl">
                Statement pairs engineered for comfort, movement, and bold silhouettes.
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {loading && shoes.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-white/60 mt-4">Loading shoes...</p>
          </div>
        )}
        ...
        {!loading && shoes.length === 0 && !error && (
          <div className="border border-white/10 rounded-2xl p-12 text-center bg-white/5">
            <p className="text-white/70 text-lg font-semibold">No shoes yet</p>
            <p className="text-white/50 mt-2">Add your first pair to get started.</p>
          </div>
        )}
