...
export default function ShoeDetailPage() {
  ...
  const [item, setItem] = useState<Shoe | null>(null);
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const data = await shoeService.getShoe(id);
        if (data) {
          setItem(data);
        } else {
          ...
          <div className="space-y-6">
            ...
            <div className="border border-white/10 rounded-2xl p-5 bg-white/5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-white/70">
                ...
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Sole</span>
                  <span>{item.specifications.sole}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">Color</span>
                  <span>{item.specifications.color}</span>
                </div>
                {item.specifications.closure && (
                  <div className="flex justify-between gap-3">
                    <span className="text-white/50">Closure</span>
                    <span>{item.specifications.closure}</span>
                  </div>
                )}
