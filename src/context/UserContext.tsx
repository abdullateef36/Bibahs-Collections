...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Load cached admin status instantly (prevents UI flicker)
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        try {
          ...
          // Fallback to cached admin status if token refresh fails
      ...
      // Persist admin status and display name to localStorage
      if (firebaseUser) {
  ...
  return (
    <UserContext.Provider value={{ user, loading, isAdmin, displayName }}>
      {children}
